...
from app import db
from app.models.stock_analysis import MarketAnalysisRecord, StockSymbol
from app.services.market_service import build_mock_quote, normalize_indicators, normalize_symbol
...
def _ensure_symbol(symbol):
    ...
    if StockSymbol.query.filter_by(symbol=normalized).first() is None:
        db.session.add(StockSymbol(symbol=normalized, name=normalized))
    return normalized
...
def market_quote():
    try:
        ...
        source = (request.args.get('source') or 'mock').strip() or 'mock'
        payload = build_mock_quote(
            symbol=symbol,

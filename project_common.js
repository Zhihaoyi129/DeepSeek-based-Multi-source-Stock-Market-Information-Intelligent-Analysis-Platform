(function (window) {
    ...
    function renderTable(columns, rows) {
        if (!rows || !rows.length) {
            return '<div class="empty-state" style="text-align: center; padding: 60px 20px; color: var(--text-soft);">' +
                   '<svg style="width: 48px; height: 48px; margin-bottom: 12px; opacity: 0.3; display: inline-block;" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">' +
                   '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"></path>' +
                   '</svg>' +
                   '<br><span style="font-size: 14px;">暂无分析记录，请先选择股票生成</span>' +
                   '</div>';
        }

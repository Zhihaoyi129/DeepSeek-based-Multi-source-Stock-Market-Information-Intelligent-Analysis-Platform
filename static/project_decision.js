StockPlatform.bootstrap(function ($) {
    ...
    const submitButton = document.getElementById('decisionSubmit');
    ...
    function renderDecision(data) {
        let deepseekText = data.deepseekPlaceholder || '-';
        try {
            const deepseekPayload = JSON.parse(deepseekText);
            deepseekText = deepseekPayload.message || deepseekText;
        } catch (error) {
        ...
        resultContainer.innerHTML =
            ...
            StockPlatform.renderKeyValues([
                ...
                { label: 'DeepSeek', value: deepseekText },
            ]) +
            '</div>' +
            '</div>';
    }
    ...
    form.addEventListener('submit', async function (event) {
        ...
        try {
            await StockPlatform.withButtonLoading(submitButton, '生成中...', async function () {
                StockPlatform.clearNotice();
                const payload = {
                    symbol: StockPlatform.getStockInputValue('decisionSymbol'),
    ...
    (async function init() {
        StockPlatform.initStockSelector($, 'decisionSymbol');
        try {
